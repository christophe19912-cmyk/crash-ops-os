...
function RepairWorkspace({ initialRepairId, focused = false }: RepairWorkspaceProps) {
  ...
  return <>
    ...
    <div className={focused ? "repair-workspace-layout focused" : "repair-workspace-layout browser"}>
      {!focused && <section className="repair-card-list">{filtered.length === 0 ? <div className="panel"><p>No repairs match these filters.</p></div> : filtered.map((repair) => {
        ...
        return <article className="repair-rich-card" key={repair.id}>
      ...
      {focused && <section className="panel repair-workspace-detail">{!selected ? <p>The requested repair work file could not be found.</p> : <>
